...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	// 直播间用
	private LinearLayout.LayoutParams myTabLayoutParams;
	...
	private int underlineColor = 0x00ff8800;
	...
	private int tabSelectTextSize = 18;
	private int tabTextColorSelected = 0xFF666666;
	private int tabTextColorUnselected = 0xFF666666;
	...
	private boolean hasSelectedStatus = false;

	public PagerSlidingTabStrip(Context context) {
		this(context, null);
	}

	public PagerSlidingTabStrip(Context context, AttributeSet attr) {
		this(context, attr, 0);
	}
	...
	public PagerSlidingTabStrip(Context context, AttributeSet attr,
		...
		indicatorWidth = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, indicatorWidth, dm);
		...
		TypedArray a = context.obtainStyledAttributes(attr, ATTRS);

		tabTextSize = a.getDimensionPixelSize(0, tabTextSize);
		tabTextColorSelected = a.getColor(1, tabTextColorSelected);
		tabTextColorUnselected = a.getColor(1, tabTextColorUnselected);
		...
		indicatorWidth = a.getDimensionPixelSize(
				R.styleable.PagerSlidingTabStrip_pstsIndicatorWidth,
				indicatorWidth);
		...
		myTabLayoutParams = new LinearLayout.LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT);
	...
	private void addTab(final int position, View tab) {
		...
		if (position == myIndex) {
			tabsContainer.addView(tab, position, myTabLayoutParams);
		} else {
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawRect(lineLeft + indicatorPadding, height - indicatorHeight,
				lineRight - indicatorPadding, height, rectPaint);
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageSelected(int position) {
			...
			for (int i = 0; i < tabsContainer.getChildCount(); i++) {
				...
				if (i == position) {
					// tabText.setTextSize(TypedValue.COMPLEX_UNIT_PX,
					// tabSelectTextSize);
					tabText.setTextColor(tabTextColorSelected);
					if (hasSelectedStatus) {
						tabText.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabSelectTextSize);
					}
				} else {
					// tabText.setTextSize(TypedValue.COMPLEX_UNIT_PX,
					// tabTextSize);
	...
	public void setIndicatorPadding(int indicatorLinePaddingPx) {
		this.indicatorPadding = indicatorLinePaddingPx;
		invalidate();
	}
	...
	public void setTextSelectSize(int textSizePx) {
		this.tabSelectTextSize = textSizePx;
	...
	public void setTextColorResource(int selected, int unselected) {
		this.tabTextColorSelected = getResources().getColor(selected);
		this.tabTextColorUnselected = getResources().getColor(unselected);
		updateTabStyles();
	}
	...
	public TextView getTextView(int index) {
		return (TextView) tabsContainer.getChildAt(index);
	}
}
