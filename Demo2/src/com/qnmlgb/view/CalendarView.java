...
public class CalendarView extends View {
	...
	private static String[] signDays; // 已签到的那些天
	//TODO
	//private static ArrayList<String> Days; // 当月已经签到的集合
	// private int[] currentMonthDays; // 当月总天数的数组
	private Integer[] currentMonthDays;
	//TODO
	//private Map<String,Integer> currentdays;
	private int mday;//当前日期（1-31）
	private int thisSignStatus; //传进来的签到状态，来选择显示
	...
	public void getToday(int currentday){
		mday=currentday;
	}
	
//	public void getCurrentDaysMap(Map<String, Integer> days,int signStatus,int currentday){
//		mday=currentday;
//		currentdays=days;
//		thisSignStatus=signStatus;
//	}
//	
	private void init() {
	signDays = new String[] { "1", "3", "5", "7", "9" };
	
		BitmapDrawable bmpDr = (BitmapDrawable) getResources().getDrawable(
				R.drawable.sign_chapter);
		bmp = bmpDr.getBitmap();
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		Log.e(TAG, "suface  width = " + surface.width + "heightMeasureSpec = "
				+ surface.height);
	...
	protected void onDraw(Canvas canvas) {
		...
		//计算本月已经签到的日期，转换为数组 	TODO
	/*	if(currentdays!=null){
			CurrentSignDaysToArray(currentdays);
		}*/
		...
		for (int i = 0; i < 42; i++) {
			...
			// todayIndex
			if (todayIndex != -1 && i < todayIndex && !isLastMonth(i)
					&& !isNextMonth(i)) {
				// color = surface.todayNumberColor;
				for (int x = 0; x < signDays.length; x++) {
					if ((""+date[i]).equals(signDays[x])) {

						color = surface.beforeTheDaySign;
						drawCellImage(canvas, i, date[i] + "");
						break;
					} else {
						color = surface.beforeTheDayNo;
					}
				}
			
			} else if (todayIndex != -1 && i >= todayIndex && !isLastMonth(i)
					&& !isNextMonth(i)) {
				if (date[i]==tday) {
					if(thisSignStatus!=1){
			...
			// drawCellImage(canvas, i, date[i]+"");
			// drawCellBg(canvas,date[i],color);
		}
		super.onDraw(canvas);
	}
	/**   TODO
	 * days 当前传入进来的本月截止到当天的所有日期
	 * 		将所有截止到当天的日期筛选，过滤出已经签到的日期添加到集合并转换为数组   
	 * 		key值 +1，服务器传过来的日期从0开始
	 */
//	private static void CurrentSignDaysToArray(Map<String,Integer> days) {
...
//			value=days.get(key);
//			if(value==1){
//				Days.add((Integer.parseInt(key)+1)+"");
//			}
//		}
////		signDays = (String[]) Days.toArray();
//		signDays= Days.toArray((new String[Days.size()]));
//	}

	/**
	 * 获取当月天数集合
	 */
	/*private void CalendarMonthDays() {
		...
		currentMonthDays = Days.toArray(new Integer[Days.size()]);
	...
	private void drawCellImage(Canvas canvas, int index, String text) {
		int x = getXByIndex(index);
		int y = getYByIndex(index);
		float textWidth = surface.datePaint.measureText(text);
		float textHeight = surface.cellHeight * 0.45f;
		float left = surface.cellWidth * (x - 1) + surface.borderWidth
				+ surface.cellWidth / 2 - textWidth / (float) 2;
		/*
		 * float top = surface.monthHeight + surface.weekHeight + (y - 1)
		 * surface.cellHeight + surface.borderWidth;
		 */
		float top = surface.monthHeight + surface.weekHeight + (y - 1)
				* surface.cellHeight + surface.borderWidth + surface.cellHeight
				/ (float) 2 - textHeight / (float) 5;
	...
	private void drawCellBg(Canvas canvas, int index, int color) {
		...
		float top = surface.monthHeight + surface.weekHeight + (y - 1)
				* surface.cellHeight + surface.borderWidth;
		float top1 = surface.monthHeight + surface.weekHeight + (y - 1)
				* surface.cellHeight + surface.borderWidth + surface.cellHeight
				- mCellSpace / (float) 2.7;
		/*
		 ...
		 */
		/*
		 * canvas.drawCircle(left+34, top+28, mCellSpace /(float) 2.9,
		 * surface.cellBgPaint );
		 */

		/*
		 * canvas.drawCircle(left +surface.cellWidth/2,
		 * top+surface.cellHeight/(float)2 , mCellSpace /(float) 2.9,
		 * surface.cellBgPaint );
		 */
		canvas.drawCircle(left + surface.cellWidth / 2, top1, mCellSpace
				/ (float) 2.5, surface.cellBgPaint);//
		// +surface.cellWidth/2
	}
	 ...
	 * setSelectedDateByCoor(event.getX(), event.getY()); break; case
	 * MotionEvent.ACTION_UP: if (downDate != null) { if(isSelectMore){ if
	 * (!completed) { if (downDate.before(selectedStartDate)) { selectedEndDate
	 * = selectedStartDate; selectedStartDate = downDate; } else {
	 * selectedEndDate = downDate; } completed = true; //响应监听事件
	 * onItemClickListener
	 * .OnItemClick(selectedStartDate,selectedEndDate,downDate); } else {
	 * selectedStartDate = selectedEndDate = downDate; completed = false; }
	 * }else{ selectedStartDate = selectedEndDate = downDate; //响应监听事件
	 * onItemClickListener
	 * .OnItemClick(selectedStartDate,selectedEndDate,downDate); } invalidate();
	 * }
	 * 
	 * break; } return true; }
	 */
	...
	private class Surface {
		...
		// private int notMonthColor=Color.parseColor("#FFFFFF"); //不是本月的设定颜色
		private int beforeTheDayNo = Color.parseColor("#999999"); // 当天前未签到的字体颜色
		private int beforeTheDaySign = Color.parseColor("#fa7a0c"); // 当天前已签到的字体颜色
		private int todayTextColor = Color.parseColor("#ffffff"); // 当天字体颜色
		private int todayLast = Color.parseColor("#666666"); // 当天后字体颜色

		private int btnColor = Color.parseColor("#666666");
		private int borderColor = Color.parseColor("#F24949");
		public int todayNumberColor = Color.GRAY;//
		public int cellDownColor = Color.parseColor("#CCFFFF");
		public int cellSelectedColor = Color.parseColor("#fe7907"); // 99CCFF
																	// //f24949
		...
		public void init() {
			...
			float weekTextSize = weekHeight * 0.55f;
			weekPaint.setTextSize(weekTextSize);
			weekPaint.setTypeface(Typeface.DEFAULT);
			datePaint = new Paint();
			datePaint.setColor(textColor);
			datePaint.setAntiAlias(true);
			float cellTextSize = cellHeight * 0.4f; // 字体的高度
			...
			/*
			 * //TODO cellBgPaint = new Paint(); cellBgPaint.setAntiAlias(true);
			...
			cellBgPaint.setStyle(Paint.Style.FILL);
			cellBgPaint.setColor(cellSelectedColor); // 红色圆形 F24949
		}
	}
}
