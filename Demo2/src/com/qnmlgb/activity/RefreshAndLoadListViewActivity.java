...
import com.qnmlgb.view.RefreshAndLoadListView.IRefreshAndLoadListener;
...
public class RefreshAndLoadListViewActivity extends Activity implements IRefreshAndLoadListener {
	...
	private void showListView(List<Map<String, Object>> allList) {
		if (adapter == null) {
			listView = (RefreshAndLoadListView) findViewById(R.id.mylistview);
	...
	private List<Map<String, Object>> getRefreshAndLoadData(String type) {
		for (int i = 0; i < 3; i++) {
	...
	public void onLoad() {
		handler.postDelayed(new Runnable() {
			...
			public void run() {
				...
				// 获取更多数据
				getRefreshAndLoadData("load");
				// 更新listview显示；
				showListView(allList);
				// 通知listview加载完毕(下拉刷新 或者 加载更多)
	...
	public void refresh() {
		handler.postDelayed(new Runnable() {
			...
			public void run() {
				// TODO Auto-generated method stub
				// 下拉刷新获取更多数据
				getRefreshAndLoadData("refresh");
				showListView(allList);
				// 通知listview刷新完毕 (下拉刷新 或者 加载更多)
				listView.onRefreshFinish();
			}
		}, 2000);
	}
