...
public class QuickofThreeasOneActivity extends Activity {

	private QuickIndexbarView mQuickIndexbarView; 
	private ParallaxListView mQuickListView;
	private View head; //包含 头布局的 view
	...
	private void initHeadView() {
		head = View.inflate(QuickofThreeasOneActivity.this, R.layout.layout_head, null);
		...
		head.getViewTreeObserver().addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
			...
			public void onGlobalLayout() {
				mQuickListView.setParallaxImageView(parallImageView);
				head.getViewTreeObserver().removeOnGlobalLayoutListener(this);
			}
		});
		
		mQuickListView.addHeaderView(head);
	}

	private void initView() {
		mQuickIndexbarView = (QuickIndexbarView) findViewById(R.id.quick_indexbar_view);
		mQuickListView = (ParallaxListView) findViewById(R.id.quick_listview);
		mQuickListView.setVisibility(View.VISIBLE);
		mQuickListView.setOverScrollMode(AbsListView.OVER_SCROLL_NEVER);
...
//		mData = 
		...
		// 创建适配器
		MySwipeAdaptetr mMySwipeAdaptetr = new MySwipeAdaptetr(
				QuickofThreeasOneActivity.this, mData);
		...
		public View getView(final int position, View convertView,
			...
			if (convertView == null) {
				convertView = View.inflate(mContext,
						R.layout.list_quickindex_item, null);
			}
			...
			holder.content_text.setOnLongClickListener(new OnLongClickListener() {
				...
				public boolean onLongClick(View v) {
					...
					Utils.remindUserDialog(QuickofThreeasOneActivity.this, "后悔了",
							"确定", "你确认删除?", new LoginDialogListenner() {
			...
			holder.content_text.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(unCloseSwipeViews.size()>0){
						...
						MyToast.MakeToast(QuickofThreeasOneActivity.this, "您点击了 :"+mDataList.get(position).getName());
					}
				}
			});
